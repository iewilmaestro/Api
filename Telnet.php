...
class Telnet
{
     ...
     * @return void
     */
    public function __construct($host = '127.0.0.1', $port = '23', $timeout = 100, $prompt = '', $stream_timeout = 1)
    {
        $this->host = $host;
        $this->port = $port;
    ...
    public function exec($command, $add_newline = TRUE)
    {
        ...
        while (preg_match("/(--More--)/", $this->buffer)) {
            $this->buffer = preg_replace('/(--More--)/', "", $this->buffer);
            $this->moreLine();
  	...
  	public function execReboot()
    {
     	$this->setPrompt('[yes/no]:');

        $this->write('reboot', true);
        $this->waitPrompt();
    ...
    protected function moreLine($add_newline = TRUE)
    {
        $this->nextLine('', $add_newline);

        $until_t = time() + $this->timeout;
        do {
            // time's up (loop can be exited at end or through continue!)
            if (time() > $until_t) {
                throw new Exception("Couldn't find the requested : '$this->prompt' within {$this->timeout} seconds");
            }
            ...
            if ((!empty($prompt) && preg_match("/{$prompt}$/", $this->buffer)) || preg_match("/(--More--)$/", $this->buffer)) {
    ...
    protected function nextLine($add_newline = TRUE)
    {
        if (!$this->socket) {
            throw new Exception("Telnet connection closed");
        }
    ...
    protected function getBuffer()
    {
        ...
        $buf =  preg_replace('/\r\n|\r/', "\n", $this->buffer);
        $buf = json_decode(str_replace('\b', '', json_encode($buf)));
        $buf = json_decode(str_replace('\n\n', '\n', json_encode($buf)));
        $buf = json_decode(str_replace('\n\u0000\n', '\n', json_encode($buf)));
